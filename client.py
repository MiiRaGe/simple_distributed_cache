...
if __name__ == '__main__':
    ...
    #             raise
    #
    # print('Sleeping for 15seconds, time to kill a server')
    # sleep(15)
    ...
    for request in range(0, number_of_tests):
        ...
        if operation == 'set':
            ...
            data[random_key] = random_data
            print('Set key=%s, value=%s' % (random_key, random_data))
            client.set(random_key, random_data)
        else:
            random_key = random.choice(list(data.keys()))
            print('Get key=%s' % random_key)
            received_data = client.get(random_key)
            try:
                assert received_data == data[random_key]
            except:
                print('%s request FAILED' % request)
                print(received_data, data[random_key])
                raise
